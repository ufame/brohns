...
public plugin_init() {
	register_plugin("Knife stab info", "1.0.3", "OpenHNS"); // Kilabeez, WessTorn
...
public rgTraceAttack(id, attacker, Float:flDamage, Float:vecDir[3], tracehandle) {
	...
	if(eInfo[S_FLASHED] || eInfo[S_DISTANCE] >= 30.0) {
		...
		for(new i, target; i < pnum; i++) {
			target = players[i];
			if(target != id && target != attacker)
				client_print_color(target, print_team_blue, "^1[^4BroHNS^1] ^3%n^1 %s ^3%n^1 (dist: ^3%.2f^1 hit: ^3%s^1)", attacker, g_szTypegroup[eInfo[S_TYPESTAB]], id, eInfo[S_DISTANCE], g_szHitgroup[eInfo[S_HIT]]);
		}
	}

	client_print_color(id, print_team_blue, "^1[^4BroHNS^1] ^3%n^1 %s ^3you^1. (dist: ^3%.2f^1 hit: ^3%s^1)", attacker, g_szTypegroup[eInfo[S_TYPESTAB]], eInfo[S_DISTANCE], g_szHitgroup[eInfo[S_HIT]]);
	
	client_print_color(attacker, print_team_blue, "^1[^4BroHNS^1] ^3You^1 %s ^3%n^1 (dist: ^3%.2f^1 hit: ^3%s^1)", g_szTypegroup[eInfo[S_TYPESTAB]], id, eInfo[S_DISTANCE], g_szHitgroup[eInfo[S_HIT]]);
...
stock get_type_stab(id, Float:flDamage, Float:flDistance, bool:Flashed) {
	if (Flashed)
	    ...
	    flDamage == 195.0 ||
	    flDamage == 780.0 //Backstab in head
	) 
		return 2;
